...
class MDP:
    ...
    def __init__(self, sol, etats, actions, modele_transition, recompenses, escompte):
        ...
        self.actions = actions                          # dict etat => tuple d'actions (str)
        self.modele_transition = modele_transition      # dict (etat,action) => liste de paires (etat suivant, probabilité)
        self.recompenses = recompenses                  # ndarray des récompenses
        self.escompte = escompte                        # facteur d'escompte (float)
...
class SerpentsEchelles(MDP):
    ...
    def __init__(self, sol, passages_serpents_echelles, recompenses, escompte):
        ...
        MDP.__init__(self, sol, etats, actions, modele_transition, recompenses, escompte)
...
def test_mdp(solution, validation_file, escompte=0.9):
    ...
    sol = imp.load_source(name, solution)

    mdp = SerpentsEchelles(sol, passages_serpents_echelles, recompenses, escompte)

    # Charger solution attendue
    with open(validation_file, 'rb') as f:
        plan_test_cmp, valeur_test_cmp, plan_cmp, valeur_cmp = pickle.load(f, encoding='latin1')
    ...
    print('Ton algorithme retourne le plan (politique) suivant')
    ...
    if not compare_plans(plan, plan_cmp, cases_sans_action):
        ...
        print('')
        print('Ton algorithme calcule la valeur suivante à ce plan')
        print(valeur)
...
def main():
    ...
    if not os.path.isfile(solution):
        parser.error("-solution '{0}' doit être un fichier!".format(os.path.abspath(solution)))
