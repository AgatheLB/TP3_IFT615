...
def generate_model_observation(prob_correct=0.9):
    ...
    for i in range(NB_LETTERS):
        ...
        for neighbor in neighbors:
            p_observation[LETTERS2INT[neighbor], i] = (1. - prob_correct) / len(neighbors)
...
def Corrector_factory(corrector):
    if corrector.endswith('.py'):
        ...
        solution = imp.load_source(name, corrector)
...
def correct_words(corrector, words):
    ...
    for word in words:
        ...
        print((str.ljust(word, max_letters) + ' -> ' + str.ljust(corrected_word, max_letters) + ' p(mot_corrige,mot)={0}'.format(prob)))
...
def evaluation(letters_corrector, Corrector, validation_file):
    ...
    for word, true_corrected_word, true_prob in zip(words, corrected_words, probs):
        ...
        print(('Solution attendue:', true_corrected_word + ', p(mot_corrige,mot)=' + str(true_prob)))
    ...
    bits_corrector = Corrector(p_init_bits, p_transition_bits, p_observation_bits, int2bits, bits2int)
...
def buildArgsParser():
    ...
    p.add_argument('-init', dest="init_file", action='store', type=str, required=False, default="p_init.pkl",
                   help="fichier pickle contenant les probabilités initiales de l'état caché " +
                        "à la première position. Défaut: 'p_init.pkl'")

    p.add_argument('-transition', dest="transition_file", action='store', type=str, required=False,
                   default="p_transition.pkl",
                   help="fichier pickle contenant le modèle de transitions. Défaut: 'p_transition.pkl'")
...
def main():
    ...
    if not os.path.isfile(corrector_file):
        parser.error("-correcteur '{0}' doit être un fichier!".format(os.path.abspath(corrector_file)))

    if not os.path.isfile(init_file):
        parser.error("-init '{0}' must be an existing file!".format(os.path.abspath(init_file)))

    if not os.path.isfile(transition_file):
        parser.error("-transition '{0}' must be an existing file!".format(os.path.abspath(transition_file)))

    # Charger le modèle de transition
    with open(transition_file, 'rb') as f:
        p_transition = pickle.load(f, encoding='latin1')

    # Charger les probabilités initiales
    with open(init_file, 'rb') as f:
        p_init = pickle.load(f, encoding='latin1')

    p_observation = generate_model_observation()
    Corrector = Corrector_factory(corrector_file)
    letters_corrector = Corrector(p_init, p_transition, p_observation, INT2LETTERS, LETTERS2INT)
